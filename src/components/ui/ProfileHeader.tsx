 ...
 */
export default function ProfileHeader({ profile, availableTooltip }: ProfileHeaderProps) {
    const keywords = profile.resolved_keywords || [];
    return (
        <header className="text-center mb-10 w-full mt-8">
            <div className="relative inline-block mb-6">
                <img
                    src={profile.avatar_url || '/og-default.png'}
                    alt={`Profile Picture ${profile.full_name}`}
                    title={`Profile Picture ${profile.full_name}`}
